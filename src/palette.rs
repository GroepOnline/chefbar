...
pub fn apply_frecency_boost(action: &Action, frecency: &HashMap<String, (u32, String)>) -> i32 {
    ...
    for (key, (_count, ts)) in frecency.iter() {
        ...
        // New records use the stable action id; older callers may still have
        // title/keyword keys, so retain that compatibility path.
...
fn parse_rfc3339_to_epoch(s: &str) -> Option<u64> {
    let s = s.trim();
    // `frecency.rs` intentionally stores epoch seconds: keep that local,
    // compact format first-class while retaining compatibility with older
    // RFC3339 fixtures.
