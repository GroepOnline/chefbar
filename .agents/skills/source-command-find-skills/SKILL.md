---
name: "source-command-find-skills"
...
# source-command-find-skills

Use this skill when the user asks to run the migrated source command `find-skills`.
...
3. Install **project-local for all agents**, symlink (niet Codex-only copy, niet user-home `-g` — cloud/CI moeten de lockfile kunnen restoren):
...
7. Custom ChefBar skills belong in `.agents/skills/` (use `/chefbar-new-skill` / `skill-creator`) with a `.cursor/skills/<name>` symlink, not as a random GitHub copy and not as a Codex-native-only tree.
